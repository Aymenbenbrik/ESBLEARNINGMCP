  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Track your progress, quiz performance, and learning outcomes.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Quizzes" value={String(data.stats.quizzes_completed)} subText="Completed" />
        <StatCard label="Avg score" value={`${data.stats.avg_score.toFixed(1)}%`} />
        <StatCard label="Completion" value={`${data.stats.completion_rate.toFixed(1)}%`} subText="Across your courses" />
      </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-right py-2 px-3 text-sm text-muted-foreground">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {data.courses.map((c) => (
                    <tr key={c.id} className="border-b last:border-b-0">
                      ...
                      <td className="py-2 px-3 text-right">
                        <Button asChild size="sm" variant="outline">
                          <Link href={`/courses/${c.id}`}>Module dashboard</Link>
                        </Button>
                      </td>
                    </tr>
                  ))}
